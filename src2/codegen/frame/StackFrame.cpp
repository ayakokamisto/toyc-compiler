...
void StackFrame::setSavesReturnAddress(bool enabled) {
    requireMutable(finalized_);
    savesReturnAddress_ = enabled;
}

void StackFrame::setUsesFramePointer(bool enabled) {
    requireMutable(finalized_);
    usesFramePointer_ = enabled;
}
...
void StackFrame::finalize() {
    ...
    const int baseSavedRegisterCount =
        (savesReturnAddress_ ? 1 : 0) + (usesFramePointer_ ? 1 : 0);
    const int savedRegisterBytes =
        (baseSavedRegisterCount + static_cast<int>(calleeSavedRegs_.size())) * kWordBytes;
...
bool StackFrame::savesReturnAddress() const {
    return savesReturnAddress_;
}

bool StackFrame::usesFramePointer() const {
    return usesFramePointer_;
}
