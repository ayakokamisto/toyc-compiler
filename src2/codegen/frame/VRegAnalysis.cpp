...
void noteLoopCarriedVRegs(
    ...
    for (std::size_t blockIndex = 0; blockIndex < blocks.size(); ++blockIndex) {
        for (const std::string& successor : blockFacts[blockIndex].successors) {
            ...
            const std::size_t headerIndex = targetIt->second;
            if (headerIndex > blockIndex) {
                continue;
            }

            const BlockFacts& headerFacts = blockFacts[headerIndex];
            for (std::size_t loopIndex = headerIndex; loopIndex <= blockIndex; ++loopIndex) {
                const BlockFacts& loopFacts = blockFacts[loopIndex];
                for (const std::string& def : loopFacts.defs) {
                    if (loopFacts.liveOut.find(def) != loopFacts.liveOut.end() &&
                        headerFacts.liveIn.find(def) != headerFacts.liveIn.end()) {
                        analysis.loopCarriedVRegs.insert(def);
                    }
                }
            }
        }
...
VRegAnalysis analyzeVRegs(const contract::IRFunction& function) {
    ...
    noteLoopCarriedVRegs(function.basicBlocks, blockFacts, blockIndexByLabel, analysis);
    for (std::size_t blockIndex = 0; blockIndex < function.basicBlocks.size(); ++blockIndex) {
    ...
    for (const std::string& vreg : analysis.discoveryOrder) {
        ...
        const bool loopCarried =
            analysis.loopCarriedVRegs.find(vreg) != analysis.loopCarriedVRegs.end();
